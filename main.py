...
def blue_gradient(text):
    try:
        return Colorate.Horizontal(Colors.blue_to_cyan, text)
    except:
        return Colorate.Color(Colors.blue, text)
...
async def nuke(server_id):
    try:
        ...
        if guild:
            ...
            print(blue_gradient(f"""[!] Command Used: Nuke - {channels_deleted} channels deleted, {channels_not_deleted} channels not deleted 
...
async def create_channels(server_id):
    try:
        guild = bot.get_guild(int(server_id))
        if guild:
            num_channels = int(input(blue_gradient("Enter the number of channels to create: ")))
            channel_type = input(blue_gradient("Enter channel type (text/voice): "))
            channel_name = input(blue_gradient("Enter channel name: "))
            ...
            print(blue_gradient(f"[!] Command Used: Create Channels - {channels_created} {channel_type} channels created, {channels_not_created} channels not created - Total Time taken: {end_time_total - start_time_total:.2f} seconds"))
        else:
            print((Colorate.Color(Colors.red, "[-] Guild not found.")))
    except Exception as e:
        print((Colorate.Color(Colors.red, f"[-] Error: {e}")))
...
async def spam_channel(server_id):
    try:
        guild = bot.get_guild(int(server_id))
        if guild:
            num_messages = int(input(blue_gradient("Enter the number of messages to send: ")))
            message_content = input(blue_gradient("Enter the message content or 'embed' to use config embed: "))

            include_everyone = False
            if message_content.lower() == 'embed':
                include_everyone_input = input(blue_gradient("Include @everyone ? (yes/no): ")).lower()
            ...
            print(blue_gradient(f"[!] Command Used: Spam - {num_messages} messages sent to all text channels - Total Time taken: {end_time_total - start_time_total:.2f} seconds"))
        else:
            print((Colorate.Color(Colors.red, "[-] Guild not found.")))
    except Exception as e:
        print((Colorate.Color(Colors.red, f"[-] Error: {e}")))
...
async def ban_all(server_id, bot_id):
    try:
        guild = bot.get_guild(int(server_id))
        if guild:
            confirm = input(blue_gradient("Are you sure you want to ban all members? (yes/no): ")).lower()
            if confirm == "yes":
                start_time_total = time.time()
                tasks = [
                ...
                print(blue_gradient(f"[!] Command Used: Ban All - {members_banned} members banned, {members_failed} members not banned - Total Time taken: {end_time_total - start_time_total:.2f} seconds"))
            else:
...
async def create_role(server_id):
    try:
        guild = bot.get_guild(int(server_id))
        if guild:
            num_roles = int(input(blue_gradient("Enter the number of roles to create: ")))
            role_name = input(blue_gradient("Enter the name of the role: "))
            ...
            end_time_total = time.time()  
            print(blue_gradient(f"[!] Command Used: Create Roles - {roles_created} roles created - Total Time taken: {end_time_total - start_time_total:.2f} seconds"))
        else:
            print((Colorate.Color(Colors.red, "[-] Guild not found.")))
    except Exception as e:
        print((Colorate.Color(Colors.red, f"[-] Error: {e}")))
...
async def dm_all(server_id):
    try:
        guild = bot.get_guild(int(server_id))
        if guild:
            message_content = input(blue_gradient("Enter the message to send to all members: "))
            ...
            end_time_total = time.time()  
            print(blue_gradient(f"[!] Command Used: DM All - {members_sent} messages sent, {members_fail} messages failed - Total Time taken: {end_time_total - start_time_total:.2f} seconds"))
        else:
            print((Colorate.Color(Colors.red, "[-] Guild not found.")))
    except Exception as e:
        print((Colorate.Color(Colors.red, f"[-] Error: {e}")))
...
async def kick_all(server_id, bot_id):
    try:
        guild = bot.get_guild(int(server_id))
        if guild:
            confirm = input(blue_gradient("Are you sure you want to kick all members? (yes/no): ")).lower()
            if confirm == "yes":
                start_time_total = time.time()
                tasks = [
                ...
                print(blue_gradient(f"[!] Command Used: Kick All - {members_kicked} members kicked, {members_failed} members not kicked - Total Time taken: {end_time_total - start_time_total:.2f} seconds"))
            else:
...
async def get_admin(server_id):
    try:
        guild = bot.get_guild(int(server_id))
        if guild:
            user_id_or_all = input(blue_gradient("Enter the user ID or press Enter for the entire server: "))
            ...
            if not user_id_or_all:
                ...
                end_time_total = time.time() 
                print(blue_gradient(f"[!] Command Used: Get Admin - Admin role granted to the entire server - Total Time taken: {end_time_total - start_time_total:.2f} seconds"))
                ...
                try:
                    ...
                    if target_user:
                        ...
                        print(blue_gradient(f"[!] Command Used: Get Admin - Admin role granted to the entire server - Total Time taken: {end_time_target_user - start_time_target_user:.2f} seconds"))
                    else:
...
async def change_server(server_id):
    try:
        ...
        if guild:
            ...
            new_name = input(blue_gradient(f"Enter the new server name or press enter for config name: ")) or server_config['new_name']
            new_icon = input(blue_gradient(f"Enter the URL of the new server icon or press enter for config icon: ")) or server_config['new_icon']
            new_description = input(blue_gradient(f"Enter the new server description or press enter for config descritpion: ")) or server_config['new_description']
            ...
            print(blue_gradient(f"[!] Command Used: Change Server - Server information updated successfully - Total Time taken: {end_time_guild_changer - start_time_guild_changer:.2f} seconds"))
        else:
            print((Colorate.Color(Colors.red, "[-] Guild not found.")))
    except Exception as e:
        print((Colorate.Color(Colors.red, f"[-] Error: {e}")))
...
async def spam_webhooks(guild):
    try:
        ...
        print(blue_gradient(f"[!] Command Used: Spam - {num_messages} messages sent via webhooks - Total Time taken: {end_time_target_spam - start_time_spam:.2f} seconds"))
    except Exception as e:
        print((Colorate.Color(Colors.red, f"[-] Error: {e}")))
...
bot_token = input(blue_gradient("Enter Bot Token: "))
server_id = input(blue_gradient("Enter Server ID: "))
...
@bot.event
async def on_ready():
    print(blue_gradient(f'[+] {bot.user.name} is online!'))
    print(blue_gradient(f'[+] Server ID: {server_id}'))
    ...
    Choice :  """
        colored_ascii = blue_gradient(ascii_art)
        choice = await asyncio.to_thread(input, colored_ascii)
